...
// ═══════════════════════════════════════════════════════════
// 화면각(rel) → 측정 부호 정규화
// createRomMeasurer는 rel<0을 A쪽, rel>0을 B쪽으로 잡는다. 두 손은 카메라에
// 거울상으로 잡히므로, A/B가 늘 같은 물리 방향을 가리키도록 손에 따라 반전한다.
// 반전하는 손이 두 단계에서 서로 반대인 것은 오타가 아니다 — 옆모습과 정면은
// 뒤집히는 축이 다르기 때문이다:
//   · 굽힘·폄(옆모습): 부호 기준이 왼손 → 오른손이면 반전. A=굽힘 / B=폄
//   · 편위(정면·손바닥이 카메라, 설계서 §5.2·§5.3): 손바닥을 카메라로 향하면
//     오른손은 엄지(요측)가 화면 왼쪽(−x)에 오므로 손목→MCP중심 각이 중립(−90°)보다
//     더 음수가 된다 → 요측일 때 rel<0. 즉 부호 기준이 오른손이고 왼손을 반전한다.
//     A=요측(엄지쪽) / B=척측(새끼쪽)
// 길이-비율 역산은 쓰지 않는다 — 정면 편위도 화면 평면 안 움직임이라 직접각으로 충분(§5.1 미도입).
// ═══════════════════════════════════════════════════════════
/** 굽힘·폄(옆모습) 부호 정규화 — A=굽힘(rel<0) / B=폄(rel>0) */
export const flexExtRel = (rel, hand) => (hand === 'right' ? -rel : rel);
/** 편위(정면) 부호 정규화 — A=요측(rel<0) / B=척측(rel>0) */
export const deviationRel = (rel, hand) => (hand === 'left' ? -rel : rel);
