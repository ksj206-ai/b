...
// ─── 자세(뷰) 검증 게이트 ───
// 측정이 조용히 무너지는 가장 흔한 경로: 옆모습 단계인데 손바닥을 카메라로 향하는 것.
// 지금은 그래도 중립이 잡히고, 그 뒤 각도는 전부 다른 평면의 값이 된다.
//
// 판정에 fingerMetrics.spread(너클 폭 ÷ 손바닥)를 쓴다. 손가락 외전에는 안 변하지만
// 손이 돌아 단축되면 변하는 값이라 — 외전 지표로는 못 쓰고 '뷰 지표'로는 딱이다.
//   · 정면(손바닥이 카메라): 너클 줄이 화면에 펼쳐져 spread가 크다
//   · 옆모습(손날이 카메라): 너클 줄이 시선 축으로 겹쳐 spread가 작다
//
...
export const VIEW_FIT = {
  ...
  relaxMs: 8000,    // 이 시간 동안 자세를 못 맞추면 안내를 접고 진행한다(관대함) —
                    // 대신 그 기록은 view:'off'로 남아 신호 판정에서 빠진다
};

// ─── 중립 잡기 (가이드·게임 공용) ───
// 각도(rel) 판정은 중립을 잡아야 시작된다 — tracker.neutral이 null이면 rel이 0에
// 고정돼 판정기가 영원히 안 센다. 가이드와 게임이 같은 규칙을 써야 "가이드에선
// 잡히는데 게임에선 안 잡히는" 자리가 안 생긴다. 그래서 main.js의 지역 상수였던
// 것을 여기로 올린다.
export const NEUTRAL = {
  frames: 14,     // 손 보이는 프레임이 이만큼 모이면 확정
  maxMs: 8000,    // 이 시간까지 못 모으면 아래 최소 수만 채워도 확정
  minFrames: 3,   // 타임아웃이어도 최소 이만큼은 봐야 확정 — 허공 중립 방지
};
...
export const ROUTINE = {
  ...
  nextAutoMs: 3000,      // 연속 재생: 완료 후 다음 운동 자동 시작(ms) — [다음] 버튼에 3·2·1로 보인다
                         // (2200이던 때는 초를 안 보여 줘서 쉬려는 순간 갑자기 넘어갔다)
  ...
  adaptImprove: {
    ...
    maxMeasureAgeDays: 14, // 가장 최근 측정이 이 일수 '초과'로 오래됐으면 긍정 신호 표시 안 함 —
                          // 오래된 측정으로 "좋아지고 있어요"라 하지 않는다.
                          //
                          // 신선도의 두 링크(오늘↔최신 / 최신↔직전)는 store.PAIR_MAX_GAP_DAYS(30)가
                          // red·improve 양쪽에 동시에 건다 — 옛 "한쪽만 가드하면 비대칭이 남음"
                          // 우려는 해소됐다. red에 신선도를 건 이유는 낡은 red가 안전한 게 아니라
                          // 자의적이기 때문이다: 더 직접적이고 신선한 안전 채널(사용자 본인의 stiff
                          // 보고)이 그대로 순한 코스를 발동시키므로, 끄는 건 자의적인 채널 하나뿐이다.
                          //
                          // 이 값(14)은 그 위에 improve에만 한 번 더 걸리는 **의도적 비대칭**이다 —
                          // 칭찬 도배 방지는 빡빡하게, red발 조심은 30일까지 관용. 구멍이 아니다.
  },
};
