...
export const ROUTINE = {
  ...
  adaptImprove: {
    ...
    maxMeasureAgeDays: 14, // 가장 최근 측정이 이 일수 '초과'로 오래됐으면 긍정 신호 표시 안 함 —
                          ...
                          //
                          // ⚠ 신선도는 두 가지다. 서로 다른 곳에 있으니 헷갈리지 말 것:
                          //   ① 쌍이 서로 얼마나 벌어졌나 → store.PAIR_MAX_GAP_DAYS(30).
                          //      red·improve 양쪽에 동시에 걸리므로 비대칭 없음(옛 우려는 해소됨).
                          //   ② 그 쌍이 오늘로부터 얼마나 오래됐나 → improve는 이 값(14)이
                          //      표시 단계에서 막는다. **red엔 아직 대응물이 없다** —
                          //      isRedSignal은 날짜를 아예 안 읽어서, 반년 전에 가깝게 찍힌
                          //      두 기록으로 오늘도 순한 코스가 발동한다. 여기가 남은 구멍이다.
  },
};
