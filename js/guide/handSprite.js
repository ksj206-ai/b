...
// 힘줄활주는 원본 영상이 달라 팔뚝 중심·손바닥 아래끝을 grip 정지 프레임에 맞춘 값이다.
//
// ─── 에셋 제작 ───
// 여섯 개 모두 assets/make_guide_sprites.py가 Flow 영상에서 뽑는다(원본 파일명·구간·
// 시간표가 거기 적혀 있다). 그 스크립트가 출력하는 pivot·k를 아래에 그대로 옮긴다.
// grip·spread·flexext는 8월 제작분의 프레임을 영상과 대조해 역산했다 — 움직임·배치는 그대로다.
// ─── 에셋 버전 ───
...
// 3: 편위·힘줄활주를 grip 화풍으로 교체 · 4: 핀치를 영상 끝부분 한 번으로 + 여섯 개 모두
//    배경을 다시 빼 손가락 사이 흰 틈·흰 테 제거 (2026-09-11)
const ASSET_V = 4;
...
const SPRITES = {
  ...
  tendon_glide: {
    ...
    w: 480, h: 270, pivot: { x: 240.3, y: 177.8 }, k: 0.99,
  },
};
