...
export const GAME_REGISTRY = {
  ...
  flex_ext: {
    ...
    usePose: false,
    load: () => import('./catRocket.js').then((m) => m.createCatRocket),
    idleEmoji: '🚀',
    idleTitle: '로켓이 힘을 모으고 있어요',
    // ★어느 쪽이 웅크림인지 문구로 못 박지 않는다. 판정기는 화면 기준 rel을 그대로
    //   보고(가이드와 동일) 손별 부호 정규화(flexExtRel)를 쓰지 않아서, 손을 어느 쪽으로
    //   세우느냐에 따라 굽힘·폄이 뒤집힌다. "굽히면 웅크린다"라고 적으면 절반의
    //   사용자에게 틀린 말이 된다. 불꽃이 실시간으로 반응하므로 한 번 움직여 보면 안다.
    idleText: '손목을 천천히 <b>굽혔다 폈다</b> 하면 불꽃이 세졌다 약해져요.<br>'
            + '한 번 왕복할 때마다 로켓이 한 단계 솟습니다. 오른쪽 <b>시작하기</b>를 누르면 카메라가 켜집니다.',
    lead: '손목을 천천히 굽혔다 펴면서 로켓을 밀어 올려 보세요. 오른쪽 고도계가 다 차면 오늘의 손목 굽혔다 펴기를 한 것으로 기록돼요.',
    practiceLead: '오늘 몫은 다 했어요. 이건 그냥 재미로 🚀',
    doneMsg: '궤도에 올랐어요! 오늘의 손목 굽혔다 펴기 완료 ✨ 홈에서 별을 확인해 보세요.',
    practiceMsg: '궤도에 올랐어요! 오늘 몫은 이미 끝나 있어요 🚀',
  },
  ...
  deviation: {
    ...
    // 고양이 로켓과 같은 이유로 좌/우를 문구로 못 박지 않는다(deviationRel을 안 쓴다).
    idleText: '손목을 <b>좌우로 천천히 기울이면</b> 고양이가 따라 움직여요.<br>'
