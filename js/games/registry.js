...
export const GAME_REGISTRY = {
  ...
  flex_ext: {
    ...
    usePose: false,
    load: () => import('./moonPhase.js').then((m) => m.createMoonPhase),
    idleEmoji: '🌙',
    idleTitle: '달이 손목을 따라 차고 기울어요',
    // ★어느 쪽이 차오르는 쪽인지 문구로 못 박지 않는다. 판정기는 화면 기준 rel을
    //   그대로 보고(가이드와 동일) 손별 부호 정규화(flexExtRel)를 쓰지 않아서,
    //   손을 어느 쪽으로 세우느냐에 따라 위아래가 뒤집힌다. "위로 올리면 보름"이라
    //   적으면 절반의 사용자에게 틀린 말이 된다. 달이 실시간으로 변하므로 한 번
    //   움직여 보면 안다.
    idleText: '손목을 천천히 <b>굽혔다 폈다</b> 하면 달이 차오르고 기울어요.<br>'
            + '보름과 그믐을 한 번씩 지나면 1회예요. 오른쪽 <b>시작하기</b>를 누르면 카메라가 켜집니다.',
    lead: '손목을 천천히 굽혔다 펴면서 달을 보름까지 채웠다가 그믐까지 기울여 보세요. 다 채우면 오늘의 손목 굽혔다 펴기를 한 것으로 기록돼요.',
    practiceLead: '오늘 몫은 다 했어요. 이건 그냥 재미로 🌙',
    doneMsg: '달을 다 채웠어요! 오늘의 손목 굽혔다 펴기 완료 ✨ 홈에서 별을 확인해 보세요.',
    practiceMsg: '달을 다 채웠어요! 오늘 몫은 이미 끝나 있어요 🌙',
  },
  ...
  deviation: {
    ...
    // 달 채우기와 같은 이유로 좌/우를 문구로 못 박지 않는다(deviationRel을 안 쓴다).
