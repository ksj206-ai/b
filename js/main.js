  ...
  try {
    ...
    guide.tracking.startLoop(({ hand, pose, handLabel, now }) => {
      ...
      if (nw) {
        ...
        if (nw.frames >= NEUTRAL_FRAMES || (timedOut && nw.frames >= NEUTRAL.minFrames)) {
 ...
 *  값은 config.NEUTRAL — 게임 세션(games/session.js)이 같은 규칙을 쓴다. */
const NEUTRAL_FRAMES = NEUTRAL.frames, NEUTRAL_MAX_MS = NEUTRAL.maxMs;
...
// ═══════════════════════════════════════════════════════════
// 게임 화면 — 라우터 훅만 남긴다 (배관은 games/session.js)
//
// 화면에 들어갈 때 처음으로 모듈을 불러온다. 게임 경로는 tracking·measurement·
// MediaPipe까지 끌고 오므로, 게임 탭을 한 번도 안 여는 사용자가 그 비용을
// 내지 않게 한다.
// ═══════════════════════════════════════════════════════════
let gameMod = null;
...
async function enterGame() {
  ...
  try {
    gameMod ??= await import('./games/session.js');
    gameMod.enterGame();
  } catch (err) {
    console.error('[game] 게임 모듈을 불러오지 못했습니다', err);
    const t = document.getElementById('gmIdleText');
    if (t) t.textContent = '게임을 불러오지 못했어요. 새로고침해 주세요.';
  }
}

function stopGame() {
  gameMod?.stopGame();   // 아직 안 불러왔으면 돌아갈 세션도 없다
}
